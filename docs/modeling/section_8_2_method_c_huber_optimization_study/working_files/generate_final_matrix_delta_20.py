...
def predict_charging_time(d):
    ...
    f_skin_headroom = 1.0  # Always set to 1.0
    ...
    return T_predicted, f_thermal, f_skin_headroom, F_system, P_effective, C_rate, C0_effective, C0_base

table_rows = []
for d in devices:
    T_pred, f_th, f_sk, F_sys, P_eff, C_rate, C0_eff, C0_base = predict_charging_time(d)
    err = T_pred - d["T_A"]
...
headers = [
    ...
    "TDSI Score", "`f_skin_headroom`", "Benchmark `T_A`", "Predicted `T_C`", "Residual Error (`Delta`)", "Error %"
]
alignments = [":---", ":---:", ":---:", ":---:", ":---:", ":---:", ":---:", ":---:", ":---:", ":---:", ":---:", ":---:", ":---:", ":---:", ":---:", ":---:"]
...
align_str = "| " + " | ".join(alignments[i].ljust(col_widths[i], '-') if alignments[i].endswith(':') else alignments[i].ljust(col_widths[i], '-') for i in range(len(alignments))) + " |"
...
formatted_md = "\n".join(lines_out)
with open(r"c:\Users\Ion\.gemini\antigravity\scratch\smartphone_db\docs\modeling\section_8_2_method_c_huber_optimization_study\working_files\generated_table.md", "w", encoding="utf-8") as f:
    f.write(formatted_md)

print("Generated table successfully.")
