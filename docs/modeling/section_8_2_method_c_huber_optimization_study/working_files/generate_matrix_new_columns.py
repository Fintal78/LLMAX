...
# Calibrated parameters without T_handshake under delta = 5.0 mins
eta_low_opt = 0.9679
C0_single_opt = 0.4021
C0_dual_opt = 4.9126
k_opt = 1.0576
p_opt = 0.2347
...
def predict_charging_time(d):
    ...
    return T_predicted, f_thermal, f_skin_headroom, F_system, P_effective, C_rate, C0_base, C0_effective
...
header = "| Smartphone Device Model      | Arch   | P_peak (W) | E_supply (Wh) | C_rate (h^-1) | P_eff (W) | F_system | `C0_effective` | `C0_base` | `f_thermal` | TDSI Score | `f_skin_headroom` | Benchmark `T_A` | Predicted `T_C` | Residual Error (`Delta`) | Error % |"
divider = "| :--------------------------- | :----: | :--------: | :-----------: | :-----------: | :-------: | :------: | :------------: | :-------: | :---------: | :--------: | :----------------: | :-------------: | :-------------: | :----------------------: | :-----: |"
...
for d in devices:
    T_pred, f_th, f_sk, F_sys, P_eff, C_rate, C0_b, C0_eff = predict_charging_time(d)
    err = T_pred - d["T_A"]
    err_pct = (err / d["T_A"]) * 100.0
    
    print(f"| **{d['name']}** | {d['arch']} | {d['P_peak']:.1f} W | {d['E_supply']:.2f} Wh | {C_rate:.2f} | {P_eff:.1f} W | {F_sys:.4f} | {C0_eff:.4f} | {C0_b:.4f} | {f_th:.4f} | {d['tdsi']:.2f} | {f_sk:.4f} | {d['T_A']:.1f} m | {T_pred:.1f} m | {'+' if err>=0 else ''}{err:.1f} m | {'+' if err_pct>=0 else ''}{err_pct:.1f}% |")
