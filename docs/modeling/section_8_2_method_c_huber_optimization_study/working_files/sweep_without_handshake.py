...
def predict_charging_time(d, eta_low, C0_single_base, C0_dual_base, k, p):
    ...
    # Pure physical model WITHOUT T_handshake
...
print("=== SWEEP WITHOUT T_handshake ===")
print("| Huber Threshold (`delta`) | eta_low  | C0_single (h^-1) | C0_dual (h^-1) | `k`      | `p`      | `Mean_dT` (mins) | `MAE_T` (mins) | `RMSE_T` (mins) | Max Error (mins) | Boundary Status |")
print("| :-----------------------: | :------: | :--------------: | :------------: | :------: | :------: | :--------------: | :------------: | :-------------: | :--------------: | :-------------: |")
...
for delta in deltas:
    ...
    mean_dt_val = np.mean(diffs)
    
    label = f"**`{delta:.1f}`**" if delta > 0 else "**`0.0` (Pure MAE)**"
    if delta == 5.0: label = "**`5.0` (Primary)**"
    if delta == 100.0: label = "**`100.0` (MSE-like)**"
    
    print(f"| {label:25s} | `{eta_low_opt:.4f}` | `{C0_single_opt:.4f}`         | `{C0_dual_opt:.4f}`       | `{k_opt:.4f}` | `{p_opt:.4f}` | `{mean_dt_val:+6.2f}`          | **`{mae_val:.2f}`**     | `{rmse_val:.2f}`         | `{max_err_val:.2f}`          | OK (Interior)   |")
