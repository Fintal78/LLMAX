...
# ABSOLUTE ZERO TOUCHING DOMAINS — WIDE ENOUGH THAT EVERY PARAMETER IS STRICTLY INTERIOR
PARAM_BOUNDS_ABSOLUTE = [
    ...
    (0.50, 3.50),    # F_charge_pump
    (0.50, 3.00),    # F_pps
    (0.40, 2.50),    # F_fixed_pd
    (0.40, 2.50),    # F_legacy_5v
    (0.40, 2.50),    # F_apple
    (0.60, 2.50),    # F_arch
]
...
print("=== TESTING ABSOLUTE ZERO TOUCHING DOMAINS ===")

p_mse, l_mse = global_optimize("mse", PARAM_BOUNDS_ABSOLUTE, seed=101)
p_mae, l_mae = global_optimize("mae", PARAM_BOUNDS_ABSOLUTE, seed=202)
p_hub, l_hub = global_optimize("huber", PARAM_BOUNDS_ABSOLUTE, delta=10.0, seed=303)
...
for label, p in [("Pure MSE", p_mse), ("Pure MAE", p_mae), ("Huber 10", p_hub)]:
    issues = check_interiority_margin(p, PARAM_BOUNDS_ABSOLUTE)
    if issues:
