...
# Script to execute high-precision global optimization under expanded search domains:
# Param 1 (eta_CCCV):        [0.30, 1.00]
# Param 2 (C_threshold):     [0.50, 2.50]
# Param 3 (s_low):           [0.01, 3.00]
# Param 4 (eta_arch_single): [0.70, 1.00]
# Param 5 (eta_proto_cp):    [0.50, 1.00]
# Param 6 (eta_proto_pps):   [0.50, 1.00]
# Param 7 (eta_proto_fpd):   [0.50, 1.00]
# Param 8 (eta_proto_5v):    [0.50, 1.00]
# Param 9 (eta_proto_app):   [0.50, 1.00]
# Param 10 (k):              [0.01, 2.00]
# Param 11 (p):              [0.10, 1.20]
# Param 12 (T_handshake):    [0.50, 0.50]
...
BASELINE_PARAMS_LOSS = [
    0.72,   # Param 1: eta_CCCV
    1.50,   # Param 2: C_threshold
    0.15,   # Param 3: s_low
    0.94,   # Param 4: eta_arch_single
    0.98,   # Param 5: eta_proto_cp
    0.95,   # Param 6: eta_proto_pps
    0.91,   # Param 7: eta_proto_fpd
    0.83,   # Param 8: eta_proto_5v
    0.88,   # Param 9: eta_proto_app
    0.20,   # Param 10: k
    0.45,   # Param 11: p
    0.50,   # Param 12: T_handshake
]
...
def global_optimize_expanded(loss_type, bounds, delta=10.0, num_trials=300000, seed=42):
    random.seed(seed)
    ...
    for trial in range(num_trials):
        ...
        for j in range(len(PARAM_KEYS_EXPANDED)):
            ...
            if random.random() < 0.5:
                cand.append(BASELINE_PARAMS_LOSS[j] + random.gauss(0, (hi - lo) * 0.20))
            else:
    ...
    # Multi-resolution pattern search refine
    step = 0.08
    for outer in range(6000):
        improved = False
        for j in range(len(PARAM_KEYS_EXPANDED)):
            lo, hi = bounds[j]
            if lo == hi:
                continue
            for mult in [1.0, 0.5, 0.2, 0.08, 0.02, 0.005, 0.001]:
                s = step * mult * (hi - lo)
...
def main():
    ...
    for dv in deltas:
        pd, _ = global_optimize_expanded("huber", PARAM_BOUNDS_EXPANDED, delta=dv, num_trials=100000, seed=int(dv*100))
        huber_models[dv] = pd
    ...
    # Find Huber threshold with minimum MSE_T
    ...
    # 2. Run Pure MSE and Pure MAE optimizations under expanded domain
    print("Running Pure MSE optimization...")
    p_mse, _ = global_optimize_expanded("mse", PARAM_BOUNDS_EXPANDED, seed=101)
    print("Running Pure MAE optimization...")
    p_mae, _ = global_optimize_expanded("mae", PARAM_BOUNDS_EXPANDED, seed=202)
    ...
    print("\nSUCCESS: High-precision expanded domain master data saved to scratch/expanded_domain_study_data.json!")
