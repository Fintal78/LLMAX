...
def global_optimize_instant(loss_type, bounds, delta=10.0, num_trials=1500, seed=42):
    random.seed(seed)
    ...
    for outer in range(60):
        ...
        for j in range(len(PARAM_KEYS_EXPANDED)):
            ...
            if lo == hi:
                continue
            for mult in [1.0, 0.3, 0.05, 0.005]:
                s = step * mult * (hi - lo)
...
def main():
    ...
    for dv in deltas:
        pd, _ = global_optimize_instant("huber", PARAM_BOUNDS_EXPANDED, delta=dv, seed=int(dv*100))
        huber_models[dv] = pd
    ...
    p_mse, _ = global_optimize_instant("mse", PARAM_BOUNDS_EXPANDED, seed=101)
    print("Running Pure MAE optimization...")
    p_mae, _ = global_optimize_instant("mae", PARAM_BOUNDS_EXPANDED, seed=202)
    ...
    print("\nSUCCESS: Instant expanded domain master data saved to scratch/expanded_domain_study_data.json!")
