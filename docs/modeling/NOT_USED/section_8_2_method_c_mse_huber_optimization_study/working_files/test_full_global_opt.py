...
PARAM_BOUNDS_GLOBAL = [
    ...
    (0.35, 0.65),    # eta_base
    (0.10, 0.60),    # s_low
    (0.50, 0.50),    # T_handshake (Physical constant 0.50 mins / 30 seconds)
...
def global_optimize(loss_type, bounds, delta=10.0, num_trials=60000, seed=42):
    random.seed(seed)
    ...
    step = 0.05
    for outer in range(1500):
        improved = False
        for j in range(len(PARAM_KEYS)):
            lo, hi = bounds[j]
            if lo == hi:
                continue
            for mult in [1.0, 0.5, 0.2, 0.05, 0.01]:
                s = step * mult * (hi - lo)
...
print("=== RUNNING FULL MULTI-START GLOBAL OPTIMIZATION FOR ALL LOSS FUNCTIONS ===")

p_mse, l_mse = global_optimize("mse", PARAM_BOUNDS_GLOBAL, seed=101)
p_mae, l_mae = global_optimize("mae", PARAM_BOUNDS_GLOBAL, seed=202)
p_hub, l_hub = global_optimize("huber", PARAM_BOUNDS_GLOBAL, delta=10.0, seed=303)
