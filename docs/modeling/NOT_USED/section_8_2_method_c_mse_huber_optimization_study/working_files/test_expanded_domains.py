...
# EXPANDED SEARCH DOMAINS — Guaranteeing ZERO boundary touching for ALL parameters
PARAM_BOUNDS_EXPANDED = [
    (0.30, 3.50),    # C_threshold: [0.30, 3.50]
    (0.001, 1.00),   # k: [0.001, 1.00]
    (0.05, 1.50),    # p: [0.05, 1.50]
    (0.20, 0.85),    # eta_base: [0.20, 0.85]
    (0.05, 1.00),    # s_low: [0.05, 1.00]
    (0.10, 2.50),    # T_handshake: [0.10, 2.50]
    (0.80, 1.65),    # F_charge_pump: [0.80, 1.65]
    (0.80, 1.45),    # F_pps: [0.80, 1.45]
    (0.70, 1.30),    # F_fixed_pd: [0.70, 1.30]
    (0.60, 1.25),    # F_legacy_5v: [0.60, 1.25]
    (0.60, 1.25),    # F_apple: [0.60, 1.25]
    (0.90, 1.65),    # F_arch: [0.90, 1.65]
]
...
def check_boundary_touching(params, bounds):
    issues = []
...
print("=== EXPANDED DOMAIN UNCONSTRAINED GLOBAL OPTIMIZATION ===")

p_mse, l_mse = global_optimize("mse", PARAM_BOUNDS_EXPANDED, seed=101)
p_mae, l_mae = global_optimize("mae", PARAM_BOUNDS_EXPANDED, seed=202)
p_hub, l_hub = global_optimize("huber", PARAM_BOUNDS_EXPANDED, delta=10.0, seed=303)
...
for label, p in [("Pure MSE", p_mse), ("Pure MAE", p_mae), ("Huber 10", p_hub)]:
    ...
    if issues:
        ...
        print(f"  {label}: ZERO BOUNDARY TOUCHING — PERFECT INTERIOR (Margin > 5% everywhere)!")
