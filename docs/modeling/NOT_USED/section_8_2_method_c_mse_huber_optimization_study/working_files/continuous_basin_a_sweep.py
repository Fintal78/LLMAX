...
# Continuous trajectory starting from delta=5.0
curr_params = [0.7533, 0.4026, 2.2493, 0.9688, 1.0000, 1.0000, 0.9043, 0.9938, 0.7196, 0.3933, 0.1808]

print("Continuous Basin A Trajectory across all deltas:")
...
for d in deltas:
    res = minimize(huber_loss_objective, curr_params, args=(d,), method='L-BFGS-B', bounds=BOUNDS, options={'ftol': 1e-15, 'gtol': 1e-15, 'maxiter': 5000})
    curr_params = res.x
    
    diffs_T = [predict_single(dev, curr_params)["t_pred"] - dev["t_actual_min"] for dev in BENCHMARK_DEVICES]
    ...
    diffs_S2 = []
    for dev in BENCHMARK_DEVICES:
        tp = predict_single(dev, curr_params)["t_pred"]
        sa = dev["s_actual"]
