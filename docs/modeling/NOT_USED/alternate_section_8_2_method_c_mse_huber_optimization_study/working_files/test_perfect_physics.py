...
# PHYSICALLY REASONABLE SEARCH DOMAINS (Grounded in thermodynamics & PMIC conversion physics)
PARAM_BOUNDS_PHYSICS_SANITY = [
    (0.60, 2.50),    # C_threshold: [0.60, 2.50]
    (0.01, 0.60),    # k: [0.01, 0.60]
    (0.10, 0.80),    # p: [0.10, 0.80]
    (0.35, 0.55),    # eta_base: [0.35, 0.55] (Physical CC/CV baseline efficiency)
    (0.10, 0.70),    # s_low: [0.10, 0.70] (Low-power scaling slope)
    (0.50, 0.50),    # T_handshake: Fixed 0.50 mins (30s physical negotiation offset)
    (1.00, 1.45),    # F_charge_pump: [1.00, 1.45] (Direct charge pump efficiency factor)
    (0.90, 1.35),    # F_pps: [0.90, 1.35] (USB-PD PPS efficiency factor)
    (0.75, 1.20),    # F_fixed_pd: [0.75, 1.20] (Fixed PD/QC efficiency factor)
    (0.65, 1.05),    # F_legacy_5v: [0.65, 1.05] (Legacy 5V efficiency factor)
    (0.70, 1.10),    # F_apple: [0.70, 1.10] (Apple PMIC profile efficiency factor)
    (0.95, 1.45),    # F_arch: [0.95, 1.45] (Dual-cell series architecture factor)
]
...
def global_optimize(loss_type, bounds, delta=10.0, num_trials=100000, seed=42):
    random.seed(seed)
...
def check_interiority_margin(params, bounds):
    issues = []
    for j, val in enumerate(params):
        ...
        if margin_lo < 0.05:
            issues.append(f"  !! {PARAM_KEYS[j]} = {val:.4f} TOO CLOSE TO LOWER BOUND {lo:.2f} (margin: {margin_lo*100:.1f}%)")
        if margin_hi < 0.05:
            issues.append(f"  !! {PARAM_KEYS[j]} = {val:.4f} TOO CLOSE TO UPPER BOUND {hi:.2f} (margin: {margin_hi*100:.1f}%)")
    return issues

print("=== TESTING PERFECT PHYSICAL SANITY DOMAINS ===")

p_mse, l_mse = global_optimize("mse", PARAM_BOUNDS_PHYSICS_SANITY, seed=101)
p_mae, l_mae = global_optimize("mae", PARAM_BOUNDS_PHYSICS_SANITY, seed=202)
p_hub, l_hub = global_optimize("huber", PARAM_BOUNDS_PHYSICS_SANITY, delta=10.0, seed=303)
...
for j, name in enumerate(PARAM_KEYS):
    lo, hi = PARAM_BOUNDS_PHYSICS_SANITY[j]
    print(f"{name:<28} {BASELINE_PARAMS[j]:>9.4f} {p_mse[j]:>9.4f} {p_mae[j]:>9.4f} {p_hub[j]:>9.4f}  [{lo:.2f}, {hi:.2f}]")

print("\n--- 2. INTERIORITY VERIFICATION ---")
for label, p in [("Pure MSE", p_mse), ("Pure MAE", p_mae), ("Huber 10", p_hub)]:
    issues = check_interiority_margin(p, PARAM_BOUNDS_PHYSICS_SANITY)
    if issues:
        print(f"  {label} Boundary Issues:")
        for iss in issues:
            print(iss)
    else:
        print(f"  {label}: ZERO BOUNDARY TOUCHING — ALL PARAMETERS 100% INTERIOR!")
