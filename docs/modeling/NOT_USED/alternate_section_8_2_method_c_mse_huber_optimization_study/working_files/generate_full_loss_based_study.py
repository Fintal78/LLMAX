...
# ============================================================================
# FULL 3-LOSS-FUNCTION EVALUATION FOR LOSS-BASED ARCHITECTURE (All factors <= 1.0)
# ============================================================================
...
def global_optimize(loss_type, bounds, delta=10.0, formulation="exponential", num_trials=120000, seed=42):
    random.seed(seed)
...
def main():
    print("Executing full optimization across Baseline, Pure MSE, Pure MAE, and Huber Loss...")
    p_mse, _ = global_optimize("mse", PARAM_BOUNDS_LOSS, seed=101)
    p_mae, _ = global_optimize("mae", PARAM_BOUNDS_LOSS, seed=202)
    p_hub, _ = global_optimize("huber", PARAM_BOUNDS_LOSS, delta=10.0, seed=303)

    out = {
        "params": {
            "baseline": [round(x, 4) for x in BASELINE_PARAMS_LOSS],
            "pure_mse": [round(x, 4) for x in p_mse],
            "pure_mae": [round(x, 4) for x in p_mae],
            "huber_10": [round(x, 4) for x in p_hub],
        },
        "t_metrics": {
            "baseline": compute_t_metrics(BASELINE_PARAMS_LOSS),
            "pure_mse": compute_t_metrics(p_mse),
            "pure_mae": compute_t_metrics(p_mae),
            "huber_10": compute_t_metrics(p_hub),
        },
        "s_metrics_strategy1": {
            "baseline": compute_s_metrics(map_score_s1(predict_all(BASELINE_PARAMS_LOSS))),
            "pure_mse": compute_s_metrics(map_score_s1(predict_all(p_mse))),
            "pure_mae": compute_s_metrics(map_score_s1(predict_all(p_mae))),
            "huber_10": compute_s_metrics(map_score_s1(predict_all(p_hub))),
        },
        "s_metrics_strategy2": {
            "baseline": compute_s_metrics(map_score_s2(predict_all(BASELINE_PARAMS_LOSS))),
            "pure_mse": compute_s_metrics(map_score_s2(predict_all(p_mse))),
            "pure_mae": compute_s_metrics(map_score_s2(predict_all(p_mae))),
            "huber_10": compute_s_metrics(map_score_s2(predict_all(p_hub))),
        }
    }

    # Huber Sweep
    sweep = []
    for dv in [5.0, 7.5, 10.0, 12.5, 15.0, 20.0]:
        pd, _ = global_optimize("huber", PARAM_BOUNDS_LOSS, delta=dv, num_trials=30000, seed=int(dv*100))
        tm = compute_t_metrics(pd)
        sweep.append({"delta": dv, "metrics": tm, "params": [round(x, 4) for x in pd]})
    out["huber_sweep"] = sweep

    with open("scratch/full_loss_based_study_data.json", "w") as f:
        json.dump(out, f, indent=2)

    print("Data successfully generated and saved to scratch/full_loss_based_study_data.json!")
