...
# EXPANDED SEARCH DOMAINS — WIDE ENOUGH THAT NO PARAMETER TOUCHES BOUNDARIES
PARAM_BOUNDS_NO_TOUCH = [
    (0.50, 3.00),    # C_threshold: [0.50, 3.00] (Optimum ~1.30, margin >25%)
    (0.001, 1.00),   # k: [0.001, 1.00] (Optimum ~0.20, margin >20%)
    (0.05, 1.20),    # p: [0.05, 1.20] (Optimum ~0.46, margin >30%)
    (0.20, 0.80),    # eta_base: [0.20, 0.80] (Optimum ~0.45, margin >40%)
    (0.05, 0.90),    # s_low: [0.05, 0.90] (Optimum ~0.50, margin >40%)
    (0.50, 0.50),    # T_handshake: FIXED at 0.50 mins (30s physical cable negotiation constant)
    (0.80, 1.60),    # F_charge_pump: [0.80, 1.60] (Optimum ~1.30, margin >37%)
    (0.80, 1.50),    # F_pps: [0.80, 1.50] (Optimum ~1.19, margin >44%)
    (0.70, 1.30),    # F_fixed_pd: [0.70, 1.30] (Optimum ~0.95, margin >41%)
    (0.60, 1.25),    # F_legacy_5v: [0.60, 1.25] (Optimum ~0.81, margin >32%)
    (0.60, 1.25),    # F_apple: [0.60, 1.25] (Optimum ~0.83, margin >35%)
    (0.90, 1.60),    # F_arch: [0.90, 1.60] (Optimum ~1.17, margin >38%)
]
...
def global_optimize(loss_type, bounds, delta=10.0, num_trials=90000, seed=42):
    random.seed(seed)
    ...
    # Multi-resolution pattern search fine-tuning
    step = 0.05
...
def check_boundary_margin(params, bounds):
    issues = []
    for j, val in enumerate(params):
        ...
        if margin_lo < 0.10:
            issues.append(f"  !! {PARAM_KEYS[j]} = {val:.4f} near lower bound {lo:.2f} (margin: {margin_lo*100:.1f}%)")
        if margin_hi < 0.10:
            issues.append(f"  !! {PARAM_KEYS[j]} = {val:.4f} near upper bound {hi:.2f} (margin: {margin_hi*100:.1f}%)")
    return issues

print("=== CHECKING ZERO BOUNDARY TOUCHING WITH WIDE DOMAINS ===")

p_mse, l_mse = global_optimize("mse", PARAM_BOUNDS_NO_TOUCH, seed=101)
p_mae, l_mae = global_optimize("mae", PARAM_BOUNDS_NO_TOUCH, seed=202)
p_hub, l_hub = global_optimize("huber", PARAM_BOUNDS_NO_TOUCH, delta=10.0, seed=303)
...
for j, name in enumerate(PARAM_KEYS):
    lo, hi = PARAM_BOUNDS_NO_TOUCH[j]
    print(f"{name:<28} {BASELINE_PARAMS[j]:>9.4f} {p_mse[j]:>9.4f} {p_mae[j]:>9.4f} {p_hub[j]:>9.4f}  [{lo:.2f}, {hi:.2f}]")

print("\n--- 2. BOUNDARY MARGIN CHECK (Margin > 10% Required) ---")
for label, p in [("Pure MSE", p_mse), ("Pure MAE", p_mae), ("Huber 10", p_hub)]:
    issues = check_boundary_margin(p, PARAM_BOUNDS_NO_TOUCH)
    if issues:
        print(f"  {label} Warnings:")
        for iss in issues:
            print(iss)
    else:
        print(f"  {label}: ZERO BOUNDARY TOUCHING — ALL PARAMETERS 100% INTERIOR (Margin > 10%)!")
