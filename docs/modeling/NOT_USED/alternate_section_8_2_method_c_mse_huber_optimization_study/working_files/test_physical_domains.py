...
# PHYSICALLY REASONABLE SEARCH DOMAINS
# Grounded in electrical conversion physics (conversion efficiencies 70% to 135%)
PARAM_BOUNDS_PHYSICAL = [
    (0.80, 2.20),    # C_threshold: [0.80, 2.20] (Thermal onset C-rate)
    (0.01, 0.80),    # k: [0.01, 0.80] (Thermal penalty coefficient)
    (0.10, 0.80),    # p: [0.10, 0.80] (Thermal exponent)
    (0.38, 0.55),    # eta_base: [0.38, 0.55] (Physical CC/CV baseline efficiency: 38% to 55%)
    (0.10, 0.65),    # s_low: [0.10, 0.65] (Low-power scaling slope)
    (0.50, 0.50),    # T_handshake: Fixed 0.50 mins (30s physical cable negotiation)
    (1.00, 1.35),    # F_charge_pump: [1.00, 1.35] (Direct charge pump: 1.00x to 1.35x)
    (0.95, 1.25),    # F_pps: [0.95, 1.25] (USB-PD PPS: 0.95x to 1.25x)
    (0.80, 1.15),    # F_fixed_pd: [0.80, 1.15] (Fixed PD/QC: 0.80x to 1.15x)
    (0.70, 1.00),    # F_legacy_5v: [0.70, 1.00] (Legacy 5V: 0.70x to 1.00x)
    (0.75, 1.10),    # F_apple: [0.75, 1.10] (Apple Legacy: 0.75x to 1.10x)
    (1.05, 1.45),    # F_arch: [1.05, 1.45] (Dual-cell 2S array: 1.05x to 1.45x)
]
...
print("=== TESTING PHYSICALLY BOUNDED SEARCH DOMAINS ===")

p_mse, l_mse = global_optimize("mse", PARAM_BOUNDS_PHYSICAL, seed=101)
p_mae, l_mae = global_optimize("mae", PARAM_BOUNDS_PHYSICAL, seed=202)
p_hub, l_hub = global_optimize("huber", PARAM_BOUNDS_PHYSICAL, delta=10.0, seed=303)
...
for label, p in [("Pure MSE", p_mse), ("Pure MAE", p_mae), ("Huber 10", p_hub)]:
    issues = check_interiority_margin(p, PARAM_BOUNDS_PHYSICAL)
    if issues:
