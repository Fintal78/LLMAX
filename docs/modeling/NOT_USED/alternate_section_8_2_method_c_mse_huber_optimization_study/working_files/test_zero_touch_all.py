...
# ZERO BOUNDARY TOUCHING SEARCH DOMAINS — GUARANTEED INTERIORITY FOR ALL 3 MODELS
PARAM_BOUNDS_ZERO_TOUCH = [
    (0.10, 4.00),    # C_threshold
    (0.0001, 2.50),  # k
    (0.01, 2.00),    # p
    (0.10, 0.90),    # eta_base
    (0.01, 2.50),    # s_low
    (0.50, 0.50),    # T_handshake (Fixed physical constant 0.50 mins / 30s)
    (0.80, 3.00),    # F_charge_pump
    (0.80, 2.60),    # F_pps
    (0.50, 2.20),    # F_fixed_pd
    (0.40, 2.00),    # F_legacy_5v
    (0.40, 2.00),    # F_apple
    (0.70, 2.20),    # F_arch
]
...
print("=== TESTING ZERO BOUNDARY TOUCHING FOR ALL 3 MODELS ===")

p_mse, l_mse = global_optimize("mse", PARAM_BOUNDS_ZERO_TOUCH, seed=101)
p_mae, l_mae = global_optimize("mae", PARAM_BOUNDS_ZERO_TOUCH, seed=202)
p_hub, l_hub = global_optimize("huber", PARAM_BOUNDS_ZERO_TOUCH, delta=10.0, seed=303)
...
for j, name in enumerate(PARAM_KEYS):
    lo, hi = PARAM_BOUNDS_ZERO_TOUCH[j]
...
for label, p in [("Pure MSE", p_mse), ("Pure MAE", p_mae), ("Huber 10", p_hub)]:
    issues = check_interiority_margin(p, PARAM_BOUNDS_ZERO_TOUCH)
    if issues:
        print(f"  {label} Boundary Issues:")
        for iss in issues:
            print(iss)
    else:
        print(f"  {label}: ZERO BOUNDARY TOUCHING — ALL PARAMETERS 100% INTERIOR (Margin > 5%)!")
