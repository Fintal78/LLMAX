...
# WIDE SEARCH DOMAINS — Guaranteed to avoid touching any domain limit
PARAM_BOUNDS_SUPER_WIDE = [
    ...
    (0.80, 2.50),    # F_charge_pump
    (0.80, 2.20),    # F_pps
    (0.50, 1.80),    # F_fixed_pd
    (0.40, 1.80),    # F_legacy_5v
    (0.40, 1.80),    # F_apple
    (0.70, 2.00),    # F_arch
]
...
print("=== TESTING SUPER WIDE UNCONSTRAINED DOMAIN OPTIMIZATION ===")

p_mse, l_mse = global_optimize("mse", PARAM_BOUNDS_SUPER_WIDE, seed=101)
p_mae, l_mae = global_optimize("mae", PARAM_BOUNDS_SUPER_WIDE, seed=202)
p_hub, l_hub = global_optimize("huber", PARAM_BOUNDS_SUPER_WIDE, delta=10.0, seed=303)
...
for j, name in enumerate(PARAM_KEYS):
    lo, hi = PARAM_BOUNDS_SUPER_WIDE[j]
...
for label, p in [("Pure MSE", p_mse), ("Pure MAE", p_mae), ("Huber 10", p_hub)]:
    issues = check_interiority_margin(p, PARAM_BOUNDS_SUPER_WIDE)
    if issues:
