...
# UNCONSTRAINED WIDE DOMAINS — Guaranteed to encompass true global minima for all parameters
PARAM_BOUNDS_UNCONSTRAINED = [
    (0.20, 3.50),    # C_threshold
    (0.001, 1.50),   # k
    (0.05, 1.50),    # p
    (0.15, 0.85),    # eta_base
    (0.05, 1.20),    # s_low
    (0.50, 0.50),    # T_handshake (Fixed 0.50 mins physical cable constant)
    (0.80, 2.00),    # F_charge_pump
    (0.80, 1.80),    # F_pps
    (0.60, 1.50),    # F_fixed_pd
    (0.50, 1.35),    # F_legacy_5v
    (0.50, 1.35),    # F_apple
    (0.80, 1.80),    # F_arch
]
...
def check_interiority_margin(params, bounds):
    ...
    for j, val in enumerate(params):
        ...
        if margin_lo < 0.05:
            issues.append(f"  !! {PARAM_KEYS[j]} = {val:.4f} TOUCHING LOWER BOUND {lo:.2f} (margin: {margin_lo*100:.1f}%)")
        if margin_hi < 0.05:
            issues.append(f"  !! {PARAM_KEYS[j]} = {val:.4f} TOUCHING UPPER BOUND {hi:.2f} (margin: {margin_hi*100:.1f}%)")
    return issues

print("=== TESTING UNCONSTRAINED DOMAIN OPTIMIZATION ===")

p_mse, l_mse = global_optimize("mse", PARAM_BOUNDS_UNCONSTRAINED, seed=101)
p_mae, l_mae = global_optimize("mae", PARAM_BOUNDS_UNCONSTRAINED, seed=202)
p_hub, l_hub = global_optimize("huber", PARAM_BOUNDS_UNCONSTRAINED, delta=10.0, seed=303)
...
for j, name in enumerate(PARAM_KEYS):
    lo, hi = PARAM_BOUNDS_UNCONSTRAINED[j]
...
for label, p in [("Pure MSE", p_mse), ("Pure MAE", p_mae), ("Huber 10", p_hub)]:
    issues = check_interiority_margin(p, PARAM_BOUNDS_UNCONSTRAINED)
    if issues:
