...
def global_optimize_expanded(loss_type, bounds, delta=10.0, num_trials=35000, seed=42):
    random.seed(seed)
    ...
    # Fast pattern search refine
    step = 0.05
    for outer in range(1200):
        improved = False
        for j in range(len(PARAM_KEYS_EXPANDED)):
            lo, hi = bounds[j]
            if lo == hi:
                continue
            for mult in [1.0, 0.4, 0.1, 0.02, 0.005, 0.001]:
                s = step * mult * (hi - lo)
...
def main():
    ...
    for dv in deltas:
        pd, _ = global_optimize_expanded("huber", PARAM_BOUNDS_EXPANDED, delta=dv, seed=int(dv*100))
        huber_models[dv] = pd
