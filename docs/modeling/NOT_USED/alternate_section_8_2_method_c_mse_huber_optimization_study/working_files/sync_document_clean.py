...
# Load clean results
with open("scratch/clean_optimization_results.json", "r") as f:
    data = json.load(f)
...
# Baseline definition and metrics
baseline_params = {
    'eta_CCCV': 0.7200,
    'C_thresh': 1.5000,
    's_low': 0.1500,
...
for h in huber_sweep:
    ...
    mae = h["metrics"]["MAE_T"]
    bias = -h["metrics"]["Mean_dT"]  # T_A - T_C
    if d == 5.0:
        rationale = "Lowest duration MAE_T (6.24 mins); strong outlier clipping."
    elif d == 7.5:
        rationale = "Lowest speed score MAE_S (0.2559 pts under Strategy 2)."
    elif d == 10.0:
...
params_order = [
    ...
    ("C_thresh", "C_threshold [Param 2] (Thermal Onset)", "[0.50, 3.00]"),
    ("s_low", "s_low [Param 3] (Low-Power Scaling)", "[0.00, 1.50]"),
    ("eta_arch_single", "eta_arch_single [Param 4] (Single-Cell)", "[0.60, 1.00]"),
    ("eta_cp", "eta_proto_cp [Param 5] (Direct Pump)", "[0.70, 1.00]"),
    ("eta_pps", "eta_proto_pps [Param 6] (USB-PD PPS)", "[0.60, 1.00]"),
    ("eta_pd", "eta_proto_fpd [Param 7] (Fixed PD/QC)", "[0.50, 0.95]"),
    ("eta_5v", "eta_proto_5v [Param 8] (Legacy 5V)", "[0.50, 0.95]"),
    ("eta_apple", "eta_proto_app [Param 9] (Apple Legacy)", "[0.50, 0.95]"),
...
for key, label, domain in params_order:
    ...
    if key != "T_handshake":
        o1_val = f"{opt_mse['params'][key]:.4f}"
        o2_val = f"{opt_mae['params'][key]:.4f}"
        o3_val = f"{h10['params'][key]:.4f}"
    else:
        o1_val = "0.5000"
        o2_val = "0.5000"
        o3_val = "0.5000"
...
| :-------------- | :------: | :-------------: | :-------------: | :-------------------------: |
| **Mean Squared Duration Error (`MSE_T`)**      | ` 724.19 mins^2` | `{opt_mse['metrics']['MSE_T']:>7.2f} mins^2` | `{opt_mae['metrics']['MSE_T']:>7.2f} mins^2` | `{h10['metrics']['MSE_T']:>7.2f} mins^2` |
| **Root Mean Square Duration Error (`RMSE_T`)** | `  26.91 mins`   | `{opt_mse['metrics']['RMSE_T']:>6.2f} mins`   | `{opt_mae['metrics']['RMSE_T']:>6.2f} mins`   | `{h10['metrics']['RMSE_T']:>6.2f} mins`   |
| **Mean Absolute Duration Error (`MAE_T`)**     | `  19.34 mins`   | `{opt_mse['metrics']['MAE_T']:>6.2f} mins`   | `{opt_mae['metrics']['MAE_T']:>6.2f} mins`   | `{h10['metrics']['MAE_T']:>6.2f} mins`   |
| **Mean Duration Bias (`Mean_dT = T_A - T_C`)** | ` +14.37 mins`   | `{-opt_mse['metrics']['Mean_dT']:>+6.2f} mins`   | `{-opt_mae['metrics']['Mean_dT']:>+6.2f} mins`   | `{-h10['metrics']['Mean_dT']:>+6.2f} mins`   |
| **Dynamic Predictor Minimum (`T_min,C`)**      | `  12.08 mins`   | `{opt_mse['metrics']['T_min_C']:>6.2f} mins`   | `{opt_mae['metrics']['T_min_C']:>6.2f} mins`   | `{h10['metrics']['T_min_C']:>6.2f} mins`   |
| **Dynamic Predictor Maximum (`T_max,C`)**      | ` 296.10 mins`   | `{opt_mse['metrics']['T_max_C']:>6.2f} mins`   | `{opt_mae['metrics']['T_max_C']:>6.2f} mins`   | `{h10['metrics']['T_max_C']:>6.2f} mins`   |"""
...
| **Opt 1: Pure MSE Model**                  | `{opt_mse['metrics']['Strategy_1']['MSE_S']:>8.4f} pts^2` | `{opt_mse['metrics']['Strategy_1']['RMSE_S']:>8.4f} pts` | `{opt_mse['metrics']['Strategy_1']['MAE_S']:>8.4f} pts` | `{-opt_mse['metrics']['Strategy_1']['Mean_dS']:>+8.4f} pts` |
| **Opt 2: Pure MAE Model**                  | `{opt_mae['metrics']['Strategy_1']['MSE_S']:>8.4f} pts^2` | `{opt_mae['metrics']['Strategy_1']['RMSE_S']:>8.4f} pts` | `{opt_mae['metrics']['Strategy_1']['MAE_S']:>8.4f} pts` | `{-opt_mae['metrics']['Strategy_1']['Mean_dS']:>+8.4f} pts` |
...
| **Opt 1: Pure MSE Model**                  | `{opt_mse['metrics']['Strategy_2']['MSE_S']:>8.4f} pts^2` | `{opt_mse['metrics']['Strategy_2']['RMSE_S']:>8.4f} pts` | `{opt_mse['metrics']['Strategy_2']['MAE_S']:>8.4f} pts` | `{-opt_mse['metrics']['Strategy_2']['Mean_dS']:>+8.4f} pts` |
| **Opt 2: Pure MAE Model**                  | `{opt_mae['metrics']['Strategy_2']['MSE_S']:>8.4f} pts^2` | `{opt_mae['metrics']['Strategy_2']['RMSE_S']:>8.4f} pts` | `{opt_mae['metrics']['Strategy_2']['MAE_S']:>8.4f} pts` | `{-opt_mae['metrics']['Strategy_2']['Mean_dS']:>+8.4f} pts` |
...
2. **Low-Threshold Behavior (`delta = 5.0` to `10.0 mins`):** At small threshold values (`delta <= 10.0 mins`), slow-charging legacy outliers (such as the Apple iPhone 7 Plus at 241.0 mins and Nokia 2.4 at 215.0 mins) exceed `delta` and are penalized linearly rather than quadratically. This allows the optimizer to achieve lower linear error on mainstream fast chargers (`MAE_T = 6.24 mins` at `delta = 5.0 mins`), while producing higher squared duration variance (`MSE_T = 172.14 mins^2` at `delta = 10.0 mins`).
3. **Objective Tradeoff Continuum:** There is no single setting that achieves the absolute minimum across all metrics simultaneously:
   ...
   - **`delta = 5.0 mins`:** Achieves the lowest mean absolute duration error across the Huber sweep (`MAE_T = 6.24 mins`).
   - **`delta = 7.5 mins`:** Achieves the lowest derived speed score error under Strategy 2 (`MAE_S = 0.2559 pts`, `MSE_S = 0.1177 pts^2`).
   - **`delta = 10.0 mins`:** Balanced robust threshold with `Mean_dT = -0.01 mins` population duration bias, `MAE_T = 6.84 mins`, and `MAE_S = 0.3020 pts`.
...
1. **Mean Squared Error (`MSE_T`) & Root Mean Square Error (`RMSE_T`):**
   - Pure MSE Optimization (Option 1) achieves the global minimum physical duration variance (`MSE_T = 84.51 mins^2`, `RMSE_T = 9.19 mins`), reducing squared duration variance by **88.3% compared to the unoptimized baseline (`724.19 mins^2`)**.
   - Option 3 (`delta = 10.0 mins`) yields `MSE_T = 172.14 mins^2` because large residuals on extreme slow chargers (e.g. Nokia 2.4) are penalized linearly rather than pulled down quadratically.

2. **Mean Absolute Error (`MAE_T`):**
   - Pure MAE Optimization (Option 2) yields `MAE_T = 6.46 mins`. Option 3 with `delta = 10.0 mins` achieves `MAE_T = 6.84 mins`, while `delta = 5.0 mins` achieves `MAE_T = 6.24 mins`.

3. **Population Direction Bias (`Mean_dT`):**
   - Pure MSE achieves `Mean_dT = -0.14 mins` mean population direction bias (`T_A - T_C`), and Huber `delta = 10.0 mins` achieves `Mean_dT = -0.01 mins`, indicating near-perfect balance without systematic over- or under-prediction across the population.
...
- **Option 2 (Pure MAE):** `T_min,C = 9.60 mins` (Redmi Note 12 Explorer: 210W), `T_max,C = 275.76 mins` (Nokia 2.4: 5W).
- **Option 3 (`delta = 10.0 mins`):** `T_min,C = 11.74 mins` (Redmi Note 12 Explorer: 210W), `T_max,C = 275.04 mins` (Nokia 2.4: 5W).
...
The table below details the exact evaluated physical parameters and intermediate component values derived from the 12 calibrated model parameters (`eta_CCCV = 0.5110`, `C_threshold = 1.4144`, `s_low = 0.4344`, `eta_arch_single = 0.9773`, `eta_proto_cp = 1.0000`, `eta_proto_pps = 0.9643`, `eta_proto_fpd = 0.7921`, `eta_proto_5v = 0.7751`, `eta_proto_app = 0.6894`, `k = 0.0746`, `p = 0.5000`, `T_handshake = 0.5000 mins`) for each of the 44 smartphones in the GSMArena laboratory benchmark dataset under Option 3 (`delta = 10.0 mins`):
...
3. **Objective Model Selection Tradeoffs:**
   ...
   - **Pure MAE (Option 2):** Minimizes linear error when tail errors are Laplace-distributed, achieving `MAE_S = 0.2287 pts` on Strategy 2 and `MAE_T = 6.46 mins`.
   - **Huber Loss (`delta = 5.0 mins`):** Achieves the lowest linear duration error across the Huber sweep (`MAE_T = 6.24 mins`, `RMSE_T = 9.61 mins`).
   - **Huber Loss (`delta = 7.5 mins`):** Achieves the lowest speed score prediction error across the Huber sweep under Strategy 2 (`MAE_S = 0.2559 pts`, `MSE_S = 0.1177 pts^2`).
   - **Huber Loss (`delta = 10.0 mins`, Option 3):** Provides a balanced configuration with near-zero duration bias (`Mean_dT = -0.01 mins`, `MSE_T = 172.14 mins^2`, `MAE_T = 6.84 mins`, `MAE_S = 0.3020 pts`).
   - **Huber Loss (`delta >= 30.0 mins`):** Asymptotes smoothly to the Pure MSE solution (`MSE_T = 84.67 mins^2` at `30.0 mins` to `84.51 mins^2` at `>= 40.0 mins`).
...
print("SUCCESS: Updated section_8_2_method_c_mse_huber_optimization_study.md with clean 11-parameter results!")
